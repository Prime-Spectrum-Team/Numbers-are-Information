...
\date{Version 1.2 --- April 16, 2026}
...
\begin{center}
\small
Archived on Zenodo (Concept DOI, always resolves to latest version):\\
\href{https://doi.org/10.5281/zenodo.19511136}{\texttt{doi.org/10.5281/zenodo.19511136}}
\end{center}
...
\begin{theorem}[Maximality of the 7-class partition]
\label{thm:maximality}
Among all set partitions of $\{S_0,\dots,S_6\}$, the identity
partition (7 blocks) is the unique \emph{maximal} element
(in the refinement order) satisfying both
\begin{enumerate}[noitemsep, topsep=2pt]
\item[(i)] \emph{$\otimes$-closure:} for every pair of blocks
      $(B, B')$, the set $\{\cl(a b) : a \in B, b \in B'\}$ lies in a
      single block (well-definedness of the induced multiplication,
      Theorem~\ref{thm:welldef});
\item[(ii)] \emph{Preservation of the 7 deterministic addition laws}
      of Table~\ref{tab:7laws}: each law $\{S_i, S_j\} \to S_k$
      remains deterministic under the quotient, with output block
      $[S_k]$.
\end{enumerate}
Equivalently, no refinement of the 7-class partition exists within the
invariants $(\Om, p \bmod 6)$, and every strict coarsening either
violates (i) or loses at least one deterministic addition law.
\end{theorem}

\begin{proof}
(i) is Theorem~\ref{thm:welldef}; for the identity partition, (ii)
holds by inspection of Table~\ref{tab:7laws}.
For the converse, we enumerate all $\mathrm{Bell}(7)=877$ partitions of
$\{S_0,\dots,S_6\}$ exhaustively (experiment E1.0b): exactly
$7$ partitions satisfy both (i) and (ii), and these form a single
chain in the refinement order
\[
\pi_1 \prec \pi_2 \prec \pi_3 \prec \pi_4 \prec \pi_5 \prec \pi_6 \prec
\pi_7
\]
with $\pi_7$ the identity. In particular $\pi_7$ is the unique maximal
element; every strict coarsening (any of $\pi_1,\dots,\pi_6$, or any
of the remaining $870$ partitions) either fails (i) or collapses at
least one deterministic addition law of Table~\ref{tab:7laws}.
Full data in \texttt{results/exp\_01\_0b\_partition\_lattice.json}. \qed
\end{proof}

\emph{Remark (multiplication-only sub-quotient).}
In the multiplicative direction alone, the four prime classes
$S_1,S_2,S_3,S_4$ share identical Cayley rows, so the $\otimes$-closed
$4$-element quotient $\{S_0, S_\mathrm{prime}, S_5, S_6\}$ also
exists. It appears in the enumeration above as a $\otimes$-closed
partition that is \emph{not} among the $7$ chain elements: it collapses
$S_3{+}S_3 \to S_5$ and $S_1{+}S_1 \not\to$\,constant into the same
block pair, violating (ii). The full 7-class refinement is therefore
necessary precisely when addition laws are considered.
...
\begin{definition}[Trivial vs.\ non-trivial determinism]\label{def:trivial-det}
A deterministic addition law $\{C_1, C_2\} \to C_3$ is \emph{trivial}
if at least one of $C_1, C_2$ is a singleton class
($S_0{=}\{1\}$, $S_3{=}\{2\}$, or $S_4{=}\{3\}$),
in which case determinism reduces to an arithmetic identity between
two fixed integers. Otherwise both operand classes are infinite and
determinism is an emergent statistical property; such laws are called
\emph{non-trivial}.
\end{definition}

\begin{theorem}[Deterministic addition laws]\label{thm:7laws}
Among the $\binom{7}{2}+7=28$ unordered class pairs, exactly
\textbf{one non-trivial} and \textbf{six trivial} deterministic
addition laws exist (Definition~\ref{def:trivial-det},
Table~\ref{tab:7laws}). The unique non-trivial law is Solar$+$Lunar
(Theorem~\ref{thm:solar-lunar}).
\end{theorem}

\begin{table}[htbp]
\centering
\caption{The 7 deterministic addition laws, split by
Definition~\ref{def:trivial-det}.}\label{tab:7laws}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}llllp{2.4cm}@{}}
\toprule
$C_1$ & $C_2$ & Result & Type & Justification \\
\midrule
$S_0$ & $S_0$ & $S_3$ & trivial     & $1+1=2$ \\
$S_0$ & $S_3$ & $S_4$ & trivial     & $1+2=3$ \\
$S_0$ & $S_4$ & $S_5$ & trivial     & $1+3=4=2^2$ \\
$S_3$ & $S_3$ & $S_5$ & trivial     & $2+2=4=2^2$ \\
$S_3$ & $S_4$ & $S_2$ & trivial     & $2+3=5$ (Lunar) \\
$S_4$ & $S_4$ & $S_5$ & trivial     & $3+3=6=2{\times}3$ \\
$\mathbf{S_1}$ & $\mathbf{S_2}$ & $\mathbf{S_6}$ & \textbf{non-trivial} &
\textbf{Thm.~\ref{thm:solar-lunar}} \\
\bottomrule
\end{tabular}
\end{table}
...
\item E0.0 (SpectralMemory benchmark): 2-layer, 4-head architecture on
      TinyShakespeare~\cite{Karpathy2015CharRNN}; 5 tests (context scaling, latency, memory,
      infinite retrieval, sliding-window comparison); see Section~\ref{subsec:structural}
\item E1.0 (Maximality of 7-class partition, illustrative): confirmed
      via three-step argument (multiplicative equivalence of
      $S_1{\equiv}S_2{\equiv}S_3{\equiv}S_4$, additive
      $S_1 \not\equiv S_2$, vacuity of $S_5/S_6$ splits)
\item E1.0b (Maximality of 7-class partition, exhaustive): all
      $\mathrm{Bell}(7){=}877$ set partitions of
      $\{S_0,\dots,S_6\}$ enumerated; 68 are
      $\otimes$-closed, 7 preserve all 7 (1 non-trivial + 6 trivial,
      Definition~\ref{def:trivial-det}) deterministic addition laws,
      and these form a chain with the identity 7-partition as the
      unique maximum (Theorem~\ref{thm:maximality})
...
\item E5.3a (Classification recovery from SA): 100{,}000 randomly
      generated $\B$-smooth values (basis $\{2,3,5,7,11,13,17\}$,
      exponents $0$--$4$) yielded 56{,}282 distinct integers
      (birthday collisions due to finite sample space $5^7{=}78{,}125$);
      $\cl(n)$ via direct factorization agrees with $\cl$ recovered from
      $\SA(n)$ in 100\% of cases (0 disagreements on all 56{,}282
      unique values)
...
\item E10.0 (All 28 addition laws): 100{,}000 pairs each; 1 non-trivial
      and 6 trivial (singleton-induced) deterministic laws confirmed,
      21 probabilistic with explicit witnesses
\end{itemize}
...
$^{b}$ Includes trial division over $K=7$ basis primes; reference value
measured on Intel Core~i5. Reproduction on AMD Ryzen~9~3900 (2~cores in
container, GCC~14.2, \texttt{-O3 -march=native}) yields 16.0\,M ops/s
(within hardware variance, single-core throughput-bound).\\
$^{c}$ Component-wise minimum on \emph{precomputed} SA vectors (memory-bandwidth-bound,
$K{=}7$ component-wise min); measured 1{,}140.3\,M ops/s on AMD Ryzen~9~3900
(GCC~14.2, \texttt{-O3 -march=native}). Combined SA-compute+GCD is bounded by
the $O(K)$ trial-division in $^{b}$; the GCD step itself is near memory bandwidth.
}
...
7 add.\ laws      & 1 non-triv.\ + 6 triv.\ det.\ & Proved \\
...
A. Karpathy,
``char-rnn: Multi-layer Recurrent Neural Networks for character-level
language models,'' GitHub repository, 2015, MIT License.
\url{https://github.com/karpathy/char-rnn}.
The Tiny Shakespeare corpus used in experiment~E0.0 (underlying text in
the public domain; concatenation distributed under the MIT License) is
retrieved from this repository at runtime and is not redistributed with
this work.
